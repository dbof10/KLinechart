...
import { type CandleBarColor, CandleType, PolygonType, type RectStyle } from "../common/Styles";
...
function getBarColor(index: number, totalData: KLineData[], styles: CandleBarColor): string[] {
  ...
  if (index < 2) {
    ...
    const itemAtI: KLineData = totalData[index];
    const itemAtMinus1: KLineData = totalData[index - 1];
    const itemAtMinus2: KLineData = totalData[index - 2];
  ...
  override drawImp(ctx: CanvasRenderingContext2D): void {
    ...
    if (candleBarOptions !== null) {
      ...
      this.eachChildren((data: VisibleData, barSpace: BarSpace) => {
        const totalData = chartStore.getDataList();
        const { data: kLineData, x } = data;
        if (isValid(kLineData)) {

          const { open, high, low, close } = kLineData;
          const { type, styles } = candleBarOptions;
          const colors: string[] = getBarColor(data.dataIndex, totalData, styles);
