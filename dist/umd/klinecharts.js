...
var COLOR_DEMAND = "rgb(6, 122, 180)";
var COLOR_SUPPLY = "rgb(210, 56, 48)";
var COLOR_LIGHT_GRAY = "#888888";
var COLOR_DEMAND_ALPHA = "rgb(6, 122, 180, 0.3)";
var COLOR_SUPPLY_ALPHA = "rgb(210, 56, 48, 0.3)";
...
function logTag() {
    log('%c❤️ Welcome to klinecharts. Version is 9.8.8', 'border-radius:4px;border:dashed 1px #1677FF;line-height:70px;padding:0 20px;margin:16px 0;font-size:14px;color:#1677FF;', '', '', '', '');
}
...
(function (ActionType) {
    ...
    ActionType["onRightClick"] = "onRightClick";
...
var KeyboardEventTracker = /** @class */ (function () {
    function KeyboardEventTracker() {
        this.isShiftPressed = false;
        // Bind the event handlers
        document.onkeydown = this.handleKeyDown.bind(this);
        document.onkeyup = this.handleKeyUp.bind(this);
    }
    KeyboardEventTracker.getInstance = function () {
    ...
    KeyboardEventTracker.prototype.handleKeyDown = function (event) {
        if (event.shiftKey) {
            this.isShiftPressed = true;
        }
    };
    KeyboardEventTracker.prototype.handleKeyUp = function (event) {
        if (!event.shiftKey) {
            this.isShiftPressed = false;
        }
    };
    KeyboardEventTracker.prototype.checkShiftKeyState = function () {
        return this.isShiftPressed;
    };
    return KeyboardEventTracker;
}());
var keyboardTracker = KeyboardEventTracker.getInstance();
...
var SHIFT_SUPPORT = ["segment", "parallelStraightLine", "priceChannelLine"];
var CONSTRAINT_OVERLAY = ["parallelStraightLine", "priceChannelLine"];
var OverlayImp = /** @class */ (function () {
    ...
    OverlayImp.prototype.eventMoveForDrawing = function (point, dataList) {
        var _a;
        var pointIndex = this.currentStep - 1;
        var newPoint = {};
        var isShiftPressed = keyboardTracker.checkShiftKeyState();
        if (isNumber(point.timestamp) === false) {
            return;
        }
        if (pointIndex === 2 && CONSTRAINT_OVERLAY.includes(this.name)) {
            if (isNumber(point.timestamp)) {
                if (point.timestamp > this.points[1].timestamp) {
                    newPoint.timestamp = this.points[1].timestamp;
                }
                else if (point.timestamp < this.points[0].timestamp) {
                    newPoint.timestamp = this.points[0].timestamp;
                }
                else {
            ...
            if (isNumber(point.dataIndex)) {
                if (point.dataIndex > this.points[1].dataIndex) {
                    newPoint.dataIndex = this.points[1].dataIndex;
                }
                else if (point.dataIndex < this.points[0].dataIndex) {
                    newPoint.dataIndex = this.points[0].dataIndex;
                }
                else {
    ...
    OverlayImp.prototype.eventPressedPointMove = function (point, pointIndex, data) {
        var _a;
        var isShiftPressed = keyboardTracker.checkShiftKeyState();
        if (pointIndex === 2 && CONSTRAINT_OVERLAY.includes(this.name)) {
            if (isNumber(point.dataIndex)) {
                if (point.dataIndex >= this.points[1].dataIndex) {
                    this.points[2].dataIndex = this.points[1].dataIndex;
                    this.points[2].timestamp = this.points[1].timestamp;
                }
                else if (point.dataIndex < this.points[0].dataIndex) {
                    this.points[2].dataIndex = this.points[0].dataIndex;
                    this.points[2].timestamp = this.points[0].timestamp;
                }
                else {
                    this.points[2].dataIndex = point.dataIndex;
                    this.points[2].timestamp = point.timestamp;
                }
            }
        }
        else {
...
var relativeStrengthIndex = {
    ...
    calc: function (dataList, indicator) {
        ...
        return dataList.map(function (kLineData, i) {
            ...
            var p = params[index];
            if (tmp > 0) {
                sumCloseAs[index] = ((_b = sumCloseAs[index]) !== null && _b !== void 0 ? _b : 0) + tmp;
            }
            else {
                sumCloseBs[index] = ((_c = sumCloseBs[index]) !== null && _c !== void 0 ? _c : 0) + Math.abs(tmp);
            }
            if (i >= p - 1) {
                ...
                var agoPreData = (_d = dataList[i - p]) !== null && _d !== void 0 ? _d : agoData;
...
function getVolumeFigure$4() {
    return {
        ...
        styles: function (data, indicator, defaultStyles) {
            var _a;
            var color = (_a = data.current.indicatorData) === null || _a === void 0 ? void 0 : _a.color;
            return { color: color };
        }
    };
}
    ...
    calc: function (dataList, indicator) {
        return dataList.map(function (kLineData, i) {
            ...
            if (i < 2) {
                ...
                var itemAtI = dataList[i];
                var itemAtMinus1 = dataList[i - 1];
                var itemAtMinus2 = dataList[i - 2];
            ...
            var vol = { volume: volume, color: color };
            return vol;
        });
    }
};
...
function getVolumeFigure$3() {
    return {
        ...
        baseValue: 0,
        styles: function (data, indicator, defaultStyles) {
            var indi = data.current.indicatorData;
            return { color: indi === null || indi === void 0 ? void 0 : indi.color };
        },
    };
}
var TWaveVolume = {
    ...
    calc: function (dataList, indicator) {
        indicator.calcParams; indicator.figures;
        var volSums = [];
        var DataStartIndex = 2; //start drawing
        var Array_Direction = Array(dataList.length).fill(-1);
...
var TBlockVolume = {
    ...
    calc: function (dataList, indicator) {
        ...
        for (var i = 0; i < dataList.length; i++) {
            ...
            for (var j = indexOfNewTrend; j <= i; j++) {
                vol += dataList[j].volume;
                var bid = dataList[j].bidVol;
                var ask = dataList[j].askVol;
...
function getPace(i) {
    var num = Math.abs(i.close - i.open);
    if (i.close === i.open) {
        return 0;
    }
    var value = 0;
    var delta = i.askVol - i.bidVol;
    value = delta * num;
    return value;
}
function getVolumeFigure$1() {
    return {
        key: "pace",
        title: "VAL: ",
        type: "bar",
...
var Period$1 = 20;
function calculateBidAsk(index, klineData) {
...
var TBidAskOscillator = {
    ...
    calc: function (dataList, indicator) {
        ...
        dataList.forEach(function (kLineData, i) {
            var bdo = {};
            if (i >= Period$1) {
                var val = calculateBidAsk(i, kLineData);
                bdo.bid = val.bid;
                bdo.ask = val.ask;
            }
            result.push(bdo);
...
var Period = 20;
function getCumDelta(index, klineData) {
    var total = 0;
    var startIndex = index - Period;
    for (var i = startIndex; i < index; i++) {
        var delta = klineData[i].askVol - klineData[i].bidVol;
        total += delta;
    }
    return total / Period;
}
function getVolumeFigure() {
    return {
        ...
        styles: function (data, indicator, defaultStyles) {
            var _a;
            return { color: (_a = data.current.indicatorData) === null || _a === void 0 ? void 0 : _a.color };
        }
    };
}
var TCumulativeDelta = {
    name: "TCD",
    ...
    calc: function (dataList, _) {
        var result = [];
        dataList.forEach(function (kLineData, i) {
            var cd = {};
            if (i >= Period) {
                var val = getCumDelta(i, dataList);
                if (val >= 0) {
...
function getIndexOfLowestValue(In, Index, Length) {
    var Low = Number.MAX_VALUE;
    var IndexAtLowest = 0;
    for (var SrcIndex = Index; SrcIndex > Index - Length; --SrcIndex) {
...
function getIndexOfHighestValue(In, Index, Length) {
    var High = Number.MIN_VALUE;
    var IndexAtHighest = 0;
    for (var SrcIndex = Index; SrcIndex > Index - Length; --SrcIndex) {
        if (SrcIndex < 0 || SrcIndex >= In.length)
            continue;
        if (In[SrcIndex] > High) {
...
var BarType;
(function (BarType) {
    BarType[BarType["Up"] = 0] = "Up";
    BarType[BarType["Down"] = 1] = "Down";
    BarType[BarType["Inisde"] = 2] = "Inisde";
    BarType[BarType["Outside"] = 3] = "Outside";
    BarType[BarType["None"] = 4] = "None";
})(BarType || (BarType = {}));

var Swing;
(function (Swing) {
    Swing[Swing["None"] = 0] = "None";
    Swing[Swing["Up"] = 1] = "Up";
    Swing[Swing["Down"] = 2] = "Down";
})(Swing || (Swing = {}));

var TextPosition;
(function (TextPosition) {
    TextPosition[TextPosition["Up"] = 0] = "Up";
    TextPosition[TextPosition["Down"] = 1] = "Down";
})(TextPosition || (TextPosition = {}));

var LIMIT_SWING_COMPARISON = 2;
var SIGNAL_STOOGE_BUY = 3000;
var SIGNAL_STOOGE_SELL = 3001;
var SIGNAL_SPRING = 3002;
var SIGNAL_UPTHRUST = 3003;
...
function calculateSellAlgo(currentSwingHighIndex, p_lastSwingHighIndices, p_lastSwingLowIndices, data) {
    var size = p_lastSwingHighIndices.length;
    var algoCount = 0;
    if (size > LIMIT_SWING_COMPARISON) {
        var prevIndex = p_lastSwingHighIndices.pop();
        var prevprevIndex = p_lastSwingHighIndices[p_lastSwingHighIndices.length - 1];
        p_lastSwingHighIndices.push(prevIndex);
        var currentBar = getBarByIndex(data[currentSwingHighIndex]);
        var prevBar = getBarByIndex(data[currentSwingHighIndex - 1]);
        var prevSwingBar = getBarByIndex(data[prevIndex]);
        var prevDeltaVolume = data[prevIndex].totalDeltaVolume;
        var prevprevDeltaVolume = data[prevprevIndex].totalDeltaVolume;
        var currentDeltaVolume = data[currentSwingHighIndex].totalDeltaVolume;
        var distanceToNearPivot = currentSwingHighIndex - p_lastSwingLowIndices[p_lastSwingLowIndices.length - 1];
        // Pullback algo
        if (p_lastSwingLowIndices.length > LIMIT_SWING_COMPARISON) {
            var prevLowIndex = p_lastSwingLowIndices.pop();
            var prevprevLowIndex = p_lastSwingLowIndices[p_lastSwingLowIndices.length - 1];
            p_lastSwingLowIndices.push(prevLowIndex);
            var prevLowDeltaVolume = data[prevLowIndex].totalDeltaVolume;
            var prevprevLowDeltaVolume = data[prevprevLowIndex].totalDeltaVolume;
            var cob = prevLowDeltaVolume < prevprevLowDeltaVolume;
            var demandDryup = currentDeltaVolume < prevDeltaVolume;
...
function calculateBuyAlgo(currentSwingLowIndex, p_lastSwingLowIndices, p_lastSwingHighIndices, data) {
    var size = p_lastSwingLowIndices.length;
    var algoCount = 0;
    if (size > LIMIT_SWING_COMPARISON) {
        var prevIndex = p_lastSwingLowIndices.pop();
        var prevprevIndex = p_lastSwingLowIndices[p_lastSwingLowIndices.length - 1];
        p_lastSwingLowIndices.push(prevIndex);
        var currentBar = getBarByIndex(data[currentSwingLowIndex]);
        var prevBar = getBarByIndex(data[currentSwingLowIndex - 1]);
        var prevSwingBar = getBarByIndex(data[prevIndex]);
        var prevDeltaVolume = data[prevIndex].totalDeltaVolume;
        var prevprevDeltaVolume = data[prevprevIndex].totalDeltaVolume;
        var currentDeltaVolume = data[currentSwingLowIndex].totalDeltaVolume;
        // Pullback algo
        var distanceToNearPivot = currentSwingLowIndex - p_lastSwingHighIndices[p_lastSwingHighIndices.length - 1];
        if (p_lastSwingHighIndices.length > LIMIT_SWING_COMPARISON) {
            var prevHighIndex = p_lastSwingHighIndices.pop();
            var prevprevHighIndex = p_lastSwingHighIndices[p_lastSwingHighIndices.length - 1];
            p_lastSwingHighIndices.push(prevHighIndex);
            var prevHighDeltaVolume = data[prevHighIndex].totalDeltaVolume;
            var prevprevHighDeltaVolume = data[prevprevHighIndex].totalDeltaVolume;
            var cob = prevHighDeltaVolume > prevprevHighDeltaVolume;
            var supplyDryup = currentDeltaVolume > prevDeltaVolume;
...
var INDEX_START = 20;
var INDEX_START_SEARCH = -1;
var PrimaryDivisor = 1;
var THRESHOLD_NO_NEW_TRENDING_BAR = 4;
var PERCENTAGE_TREND_REMAIN = 0.5;
function isReachedSwingLength(consecutiveBar, swingLength) {
    return consecutiveBar === swingLength;
}
function swingTrendUp(i, swingDirection, p_lastSwingLowIndices, p_lastSwingHighIndices, prevTrendingBarIndex, data, highs, lows) {
    if (swingDirection.value === Swing.None || swingDirection.value === Swing.Down) {
        var lastSwingHighIndex = void 0;
        ...
        calculateSwingDown(lastSwingLowIndex, lastSwingHighIndex, p_lastSwingLowIndices, p_lastSwingHighIndices, data);
...
function calculateSwingDown(lastSwingLowIndex, lastSwingHighIndex, p_lastSwingLowIndices, p_lastSwingHighIndices, data) {
...
function calculateAccumulatedVolumeSwingDown(lastSwingLowIndex, lastSwingHighIndex, data) {
    if (lastSwingLowIndex > lastSwingHighIndex && lastSwingLowIndex !== INDEX_START_SEARCH &&
        ...
        var ask = 0;
        var bid = 0;
        for (var i = lastSwingHighIndex; i <= lastSwingLowIndex; i++) {
...
function swingTrendDown(i, swingDirection, p_lastSwingHighIndices, p_lastSwingLowIndices, prevTrendingBarIndex, data, highs, lows) {
    if (swingDirection.value === Swing.None || swingDirection.value === Swing.Up) {
        ...
        calculateSwingUp(lastSwingHighIndex, lastSwingLowIndex, p_lastSwingHighIndices, p_lastSwingLowIndices, data);
        p_lastSwingHighIndices.push(lastSwingHighIndex); // add later we compare new high to previous 2 highs
    }
}
function calculateSwingUp(lastSwingHighIndex, lastSwingLowIndex, p_lastSwingHighIndices, p_lastSwingLowIndices, data) {
...
function calculateAccumulatedVolumeSwingUp(lastSwingHighIndex, lastSwingLowIndex, data) {
    if (lastSwingHighIndex > lastSwingLowIndex && lastSwingLowIndex !== INDEX_START_SEARCH &&
        ...
        var ask = 0;
        var bid = 0;
        for (var i = lastSwingLowIndex; i <= lastSwingHighIndex; i++) {
            totalVolume += data[i].volume;
            ask += data[i].askVol;
            bid += data[i].bidVol;
        }
...
function canChangeDownInExceptionConditions(i, type, prevTrendingBarIndex, lastSwingLowIndex, swingLength, data, high, low, close) {
    var cond1 = low[i] < low[lastSwingLowIndex];
    var currentSpread = high[i] - low[i];
    var prevSpread = high[i - 1] - low[i - 1];
    var currentBar = getBarByIndex(data[i]);
    var prevBar = getBarByIndex(data[i - 1]);
    var distance = i - prevTrendingBarIndex;
    var cond2 = i - lastSwingLowIndex > swingLength;
    if (cond1) {
        ...
        if (distance > THRESHOLD_NO_NEW_TRENDING_BAR) {
            var lowestCloseIdx = getIndexOfLowestValue(close, i, THRESHOLD_NO_NEW_TRENDING_BAR);
            var lowestLowIdx = getIndexOfLowestValue(low, i, THRESHOLD_NO_NEW_TRENDING_BAR);
            ...
            var lowestCloseIdx = getIndexOfLowestValue(close, i, THRESHOLD_NO_NEW_TRENDING_BAR * swingLength);
            var lowestLowIdx = getIndexOfLowestValue(low, i, THRESHOLD_NO_NEW_TRENDING_BAR * swingLength);
            var hasBiggerSpread = (currentSpread / prevSpread) >= 2;
            var trendRemainThreshold = high[i - 1] - prevSpread * PERCENTAGE_TREND_REMAIN;
            var hasCurrentBarGoodContinuation = high[i] < trendRemainThreshold;
...
function canChangeUpInExceptionConditions(i, type, prevTrendingBarIndex, lastSwingHighIndex, swingLength, data, high, low, close) {
    var cond1 = high[i] > high[lastSwingHighIndex];
    var distance = i - prevTrendingBarIndex;
    var currentSpread = high[i] - low[i];
    var prevSpread = high[i - 1] - low[i - 1];
    var currentBar = getBarByIndex(data[i]);
    var prevBar = getBarByIndex(data[i - 1]);
    var cond2 = i - lastSwingHighIndex > swingLength;
    if (cond1) {
        return currentBar.close > prevBar.close && cond2;
    }
    else {
        ...
        if (distance > THRESHOLD_NO_NEW_TRENDING_BAR) {
            var highestCloseIdx = getIndexOfHighestValue(close, i, THRESHOLD_NO_NEW_TRENDING_BAR);
            var highestHighIdx = getIndexOfHighestValue(high, i, THRESHOLD_NO_NEW_TRENDING_BAR);
            var hasBiggerSpread = (currentSpread / prevSpread) >= 1;
            ...
            var highestCloseIdx = getIndexOfHighestValue(close, i, THRESHOLD_NO_NEW_TRENDING_BAR * swingLength);
            var highestHighIdx = getIndexOfHighestValue(high, i, THRESHOLD_NO_NEW_TRENDING_BAR * swingLength);
            var hasBiggerSpread = (currentSpread / prevSpread) >= 2;
            var trendRemainThreshold = low[i - 1] + prevSpread * PERCENTAGE_TREND_REMAIN;
            var hasCurrentBarGoodContinuation = low[i] > trendRemainThreshold;
...
function calculateRealtimeSwing(i, swingLength, swingDirection, p_lastSwingHighIndices, p_lastSwingLowIndices, drawData, prevTrendingBarIndex, high, low) {
    if (swingDirection.value === Swing.Up) {
        var anchorPointIndex = INDEX_START_SEARCH;
        var lastSwingLowIndex = void 0;
        if (p_lastSwingLowIndices.length !== 0) {
        ...
        if (high[i] > high[prevTrendingBarIndex.value] || i === prevTrendingBarIndex.value || prevTrendingBarIndex.value === lastSwingLowIndex) {
            ...
            high[prevTrendingBarIndex.value];
            // if (EnableSwingDrawing) {
        ...
        if (low[i] < low[prevTrendingBarIndex.value] || i === prevTrendingBarIndex.value || prevTrendingBarIndex.value === lastSwingHighIndex) {
            ...
            low[prevTrendingBarIndex.value];
            // if (EnableSwingDrawing) {
            //   Subgraph_GannSwing[prevTrendingBarIndex] = price;
            //   ExtColorsBuffer[prevTrendingBarIndex] = 0;
            //
...
function onRender(dataList, highs, lows, closes, config) {
    ...
    var consecutiveBar = 0;
    var swingDirection = new EnumWrapper(Swing.None);
    var prevTrendingBarIndex = new NumberWrapper(INDEX_START_SEARCH);
    var p_lastSwingHighIndices = [];
    var p_lastSwingLowIndices = [];
    var drawData = {};
    for (var index = 0; index < dataList.length; index++) {
        if (index < INDEX_START) {
            continue;
        }
        var currentBar = getBarByIndex(dataList[index]);
        var prevBar = getBarByIndex(dataList[index - 1]);
        var type = getBarType(currentBar, prevBar);
        calculateRealtimeSwing(index, SwingLength, swingDirection, p_lastSwingHighIndices, p_lastSwingLowIndices, drawData, prevTrendingBarIndex, highs, lows);
        if (swingDirection.value === Swing.Up && p_lastSwingLowIndices.length !== 0) {
            var lastSwingLowIndex = p_lastSwingLowIndices[p_lastSwingLowIndices.length - 1];
            if (canChangeDownInExceptionConditions(index, type, prevTrendingBarIndex.value, lastSwingLowIndex, SwingLength, dataList, highs, lows, closes)) {
                prevDirectionalBarIndex = index;
                prevDirectionalBarType = BarType.Down;
                consecutiveBar = 0;
                swingTrendDown(index, swingDirection, p_lastSwingHighIndices, p_lastSwingLowIndices, prevTrendingBarIndex, dataList, highs);
                // DrawDebugSwingStartBar(i, "Do1", time, low);
                continue;
            }
        }
        else if (swingDirection.value === Swing.Down && p_lastSwingHighIndices.length != 0) {
            var lastSwingHighIndex = p_lastSwingHighIndices[p_lastSwingHighIndices.length - 1];
            if (canChangeUpInExceptionConditions(index, type, prevTrendingBarIndex.value, lastSwingHighIndex, SwingLength, dataList, highs, lows, closes)) {
                prevDirectionalBarIndex = index;
                prevDirectionalBarType = BarType.Up;
                consecutiveBar = 0;
                swingTrendUp(index, swingDirection, p_lastSwingLowIndices, p_lastSwingHighIndices, prevTrendingBarIndex, dataList, highs, lows);
        ...
        if (type == BarType.Up) {
            ...
            var prevDirectionalBar = getBarByIndex(dataList[prevDirectionalBarIndex]);
...
var YesterdayStructure = {
    ...
    calc: function (kLineDataList) {
        ...
        for (var i = 0; i < kLineDataList.length; i++) {
            ...
            var prevData = kLineDataList[i - 1];
            var prevDate = prevData.timestamp;
    ...
    draw: function (_a) {
        var ctx = _a.ctx, barSpace = _a.barSpace, visibleRange = _a.visibleRange, indicator = _a.indicator, xAxis = _a.xAxis, yAxis = _a.yAxis;
        var from = visibleRange.from, to = visibleRange.to;
        ...
        for (var i = from; i < to; i++) {
            ...
            var space = barSpace.gapBar / 2;
            if (data.high && data.low) {
                var yHigh = yAxis.convertToPixel(data.high);
                ctx.fillRect(x, yHigh, barSpace.bar - space, 1);
                var yLow = yAxis.convertToPixel(data.low);
...
var VWAP = {
    ...
    calc: function (kLineDataList, indicator) {
        var list = [];
        var params = indicator.calcParams;
        var numberOfDeviation = params[0];
        var iCumVolume;
        var iCumTypicalVolume;
        var v2Sum;
        var SD1 = 1.28;
        var SD2 = 2.01;
        var SD3 = 2.51;
        kLineDataList.forEach(function (e) {
            var vwap = {};
            var hl3 = ((e.high + e.low + e.close) / 3);
            ...
            var curVWAP = (iCumTypicalVolume / iCumVolume);
            var deviation = Math.sqrt(Math.max(v2Sum / iCumVolume - curVWAP * curVWAP, 0));
            vwap.vwap = curVWAP;
...
function getQuarterColor(index) {
    if (index == 1) {
...
var Quarters = {
    ...
    calc: function (dataList, indicator, alertCallback) {
        return dataList.map(function (e) {
            var index = getQuarterIndex(new Date(e.timestamp));
            var color = getQuarterColor(index);
            var q = {};
            q.color = color;
            q.index = index;
            return q;
    ...
    draw: function (_a) {
        var ctx = _a.ctx, barSpace = _a.barSpace, visibleRange = _a.visibleRange, indicator = _a.indicator, xAxis = _a.xAxis; _a.yAxis;
        var from = visibleRange.from, to = visibleRange.to;
        var result = indicator.result;
        for (var i = from; i < to; i++) {
            var data = result[i];
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (isValid(paneInstances)) {
                            ...
                            if (isValid(instance_1)) {
                                return [2 /*return*/, Promise.reject(new Error('Duplicate indicators.'))];
                            }
                        }
                        if (!isValid(paneInstances)) {
                            paneInstances = [];
    ...
    IndicatorStore.prototype.synchronizeSeriesPrecision = function (indicator) {
        ...
        var synchronize = function (instance) {
            switch (instance.series) {
                case exports.IndicatorSeries.Price: {
                    instance.setPrecision(pricePrecision, true);
                    break;
                }
                case exports.IndicatorSeries.Volume: {
...
var ruler = {
    ...
    createPointFigures: function (_a) {
        ...
        if (coordinates.length > 1) {
            ...
            var textX = (coordinates[1].x + coordinates[0].x) / 2;
            var valueDif = points[1].value - points[0].value;
            var barCount = points[1].dataIndex - points[0].dataIndex;
            var percent = valueDif / points[0].value;
            var y = coordinates[1].y;
            var color = void 0;
            if (valueDif > 0) {
            ...
            texts.push({
                x: textX,
                y: y,
                text: "".concat((percent * 100).toFixed(1), "% ").concat(barCount, " bars"),
                baseline: "bottom",
            });
            return [
                {
...
/**
 * Remove when there's api to use figure
 */
var rect$1 = {
    name: 'rect',
    totalStep: 3,
...
var arrow = {
    ...
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates;
        if (coordinates.length > 1) {
            var flag = coordinates[1].x > coordinates[0].x ? 0 : 1;
            var kb = getLinearSlopeIntercept(coordinates[0], coordinates[1]);
            var offsetAngle = void 0;
            if (kb) {
...
var circle$1 = {
    ...
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates;
        if (coordinates.length > 1) {
            var radius = getDistance$1(coordinates[0], coordinates[1]);
            return {
                type: 'circle',
                attrs: __assign(__assign({}, coordinates[0]), { r: radius }),
                styles: { style: 'stroke_fill' }
            };
        }
        return [];
...
var fibonacciSegment = {
    ...
    needDefaultYAxisFigure: true,
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates, overlay = _a.overlay, precision = _a.precision;
        var lines = [];
        var texts = [];
        if (coordinates.length > 1) {
            var textX_1 = coordinates[1].x > coordinates[0].x ? coordinates[0].x : coordinates[1].x;
            var percents = [1, 0.786, 0.618, 0.5, 0.382, 0.236, 0];
            var yDif_1 = coordinates[0].y - coordinates[1].y;
            var points_1 = overlay.points;
            var valueDif_1 = points_1[0].value - points_1[1].value;
            percents.forEach(function (percent) {
                var y = coordinates[1].y + yDif_1 * percent;
                var price = (points_1[1].value + valueDif_1 * percent).toFixed(precision.price);
...
var fibonacciExtension = {
    ...
    createPointFigures: function (_a) {
        ...
        if (coordinates.length > 2) {
            var points_1 = overlay.points;
            var valueDif_1 = points_1[1].value - points_1[0].value;
            var yDif_1 = coordinates[1].y - coordinates[0].y;
            var percents = [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1];
            var textX_1 = coordinates[2].x > coordinates[1].x ? coordinates[1].x : coordinates[2].x;
            percents.forEach(function (percent) {
                var y = coordinates[2].y + yDif_1 * percent;
                var price = (points_1[2].value + valueDif_1 * percent).toFixed(precision.price);
...
var fibonacciSpiral = {
    ...
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates, bounding = _a.bounding;
        if (coordinates.length > 1) {
            var startRadius = getDistance$1(coordinates[0], coordinates[1]) / Math.sqrt(24);
            var flag = coordinates[1].x > coordinates[0].x ? 0 : 1;
            var kb = utils.getLinearSlopeIntercept(coordinates[0], coordinates[1]);
            var offsetAngle = void 0;
            if (kb) {
                offsetAngle = Math.atan(kb[0]) + Math.PI * flag;
            }
            ...
            var rotateCoordinate1 = getRotateCoordinate({ x: coordinates[0].x - startRadius, y: coordinates[0].y }, coordinates[0], offsetAngle);
            var rotateCoordinate2 = getRotateCoordinate({ x: coordinates[0].x - startRadius, y: coordinates[0].y - startRadius }, coordinates[0], offsetAngle);
            var arcs = [__assign(__assign({}, rotateCoordinate1), { r: startRadius, startAngle: offsetAngle, endAngle: offsetAngle + Math.PI / 2 }), __assign(__assign({}, rotateCoordinate2), { r: startRadius * 2, startAngle: offsetAngle + Math.PI / 2, endAngle: offsetAngle + Math.PI })];
            ...
            for (var i = 2; i < 9; i++) {
                ...
                arcs.push(__assign(__assign({}, rotateCoordinate), { r: r, startAngle: startAngle, endAngle: endAngle }));
            }
            return [
                {
                    type: 'arc',
...
var fibonacciSpeedResistanceFan = {
    ...
    createPointFigures: function (_a) {
        ...
        var texts = [];
        if (coordinates.length > 1) {
            var xOffset_1 = coordinates[1].x > coordinates[0].x ? -38 : 4;
            var yOffset_1 = coordinates[1].y > coordinates[0].y ? -2 : 20;
            var xDistance_1 = coordinates[1].x - coordinates[0].x;
            var yDistance_1 = coordinates[1].y - coordinates[0].y;
            var percents = [1, 0.75, 0.618, 0.5, 0.382, 0.25, 0];
            percents.forEach(function (percent) {
                var x = coordinates[1].x - xDistance_1 * percent;
                var y = coordinates[1].y - yDistance_1 * percent;
                ...
                texts.unshift({
                    x: coordinates[0].x + xOffset_1,
                    y: y + 10,
                    text: "".concat(percent.toFixed(3))
                });
                texts.unshift({
                    x: x - 18,
                    y: coordinates[0].y + yOffset_1,
                    text: "".concat(percent.toFixed(3))
                });
            });
        }
...
var gannBox = {
    ...
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates;
        if (coordinates.length > 1) {
            var quarterYDis = (coordinates[1].y - coordinates[0].y) / 4;
            var xDis = coordinates[1].x - coordinates[0].x;
            var dashedLines = [
                ...
                { coordinates: [__assign({}, coordinates[0]), { x: coordinates[0].x + xDis * 0.236, y: coordinates[1].y }] },
                { coordinates: [__assign({}, coordinates[0]), { x: coordinates[0].x + xDis * 0.5, y: coordinates[1].y }] },
...
var xabcd = {
    ...
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates; _a.overlay;
        var dashedLines = [];
        var polygons = [];
        var tags = ['X', 'A', 'B', 'C', 'D'];
        var texts = coordinates.map(function (coordinate, i) { return (__assign(__assign({}, coordinate), { baseline: 'bottom', text: "(".concat(tags[i], ")") })); });
...
var anyWaves = {
    ...
    createPointFigures: function (_a) {
        var coordinates = _a.coordinates;
        var texts = coordinates.map(function (coordinate, i) { return (__assign(__assign({}, coordinate), { text: "(".concat(i, ")"), baseline: 'bottom' })); });
        return [
            {
                type: 'line',
                attrs: { coordinates: coordinates }
...
var extensions$1 = [
    ...
    simpleAnnotation, simpleTag, rect$1, arrow, circle$1, parallelogram, triangle,
...
var OVERLAY_TRADING_GROUP_ID = "OVERLAY_TRADING_GROUP_ID";
...
var OverlayStore = /** @class */ (function () {
    ...
    OverlayStore.prototype.removeInstance = function (overlayRemove) {
        ...
        var scheduleRemovedOverlays = new Set();
        if (overlayRemoveValid) {
            ...
            this._instances.forEach(function (overlays, paneId) {
                updatePaneIds.push(paneId);
                overlays.forEach(function (instance) {
                    var _a;
        ...
        if (updatePaneIds.length > 0) {
            ...
            updatePaneIds.forEach(function (paneId) {
                var overlays = _this._instances.get(paneId);
                if (overlays) {
                    var newOverlays = overlays.filter(function (element) { return !scheduleRemovedOverlays.has(paneId + element.id); });
                    _this._instances.set(paneId, newOverlays);
                }
...
var ChartStore = /** @class */ (function () {
    function ChartStore(chart, options) {
        ...
        this._tradingSettings = defaultSettings;
        this._timeFrameDuration = -1;
        this._chart = chart;
    ...
    ChartStore.prototype.addData = function (data, type, more) {
        return __awaiter(this, void 0, void 0, function () {
            var success, adjustFlag, dataLengthChange, dataCount, timestamp, lastDataTimestamp, lastBarRightSideDiffBarCount, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        success = false;
                    ...
                    case 4:
                        error_1 = _a.sent();
                        console.log("Chart store " + error_1.toString());
                        return [3 /*break*/, 5];
    ...
    ChartStore.prototype.setTradingSettings = function (settings) {
        this._tradingSettings = settings;
    };
    ChartStore.prototype.getTradingSettings = function () {
        return this._tradingSettings;
    };
    ChartStore.prototype.setTimeframeDuration = function (duration) {
        this._timeFrameDuration = duration;
    };
    ChartStore.prototype.getTimeframeDuration = function () {
        return this._timeFrameDuration;
    };
    ChartStore.prototype.addIntradayData = function (data) {
        return __awaiter(this, void 0, void 0, function () {
            var success, adjustFlag, dataLengthChange, dataCount, timestamp, lastDataTimestamp, lastBarRightSideDiffBarCount, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        success = false;
                    ...
                    case 4:
                        ...
                        console.log("Chart store " + error_2.toString());
                        return [3 /*break*/, 5];
                    case 5: return [2 /*return*/];
                }
            });
 ...
 * limitations under the License.
 */
function getBarColor(index, totalData, styles) {
    var colors = [];
    if (index < 2) {
        if (close >= open) {
        ...
        var itemAtI = totalData[index];
        var itemAtMinus1 = totalData[index - 1];
        var itemAtMinus2 = totalData[index - 2];
        if (itemAtI.close === itemAtMinus1.close) {
            if (itemAtMinus1.close > itemAtMinus2.close) {
                colors[0] = styles.upColor;
                colors[1] = styles.upBorderColor;
    ...
    CandleBarView.prototype.drawImp = function (ctx) {
        ...
        if (candleBarOptions !== null) {
            ...
            this.eachChildren(function (data, barSpace) {
                var totalData = chartStore.getDataList();
                ...
                if (isValid(kLineData)) {
                    ...
                    var colors = getBarColor(data.dataIndex, totalData, styles);
                        ...
                        eachFigures(dataList, indicator, dataIndex, defaultStyles, function (figure, figureStyles, figureIndex) {
                            ...
                            if (isValid(currentIndicatorData[figure.key])) {
                                ...
                                switch (figure.type) {
                                    ...
                                    case 'bar': {
                                        var barWidth = void 0;
                                        if (attrs === null || attrs === void 0 ? void 0 : attrs.size) {
                                            barWidth = attrs.size;
                                        }
                                        else {
                                            barWidth = gapBar;
...
var OverlayView = /** @class */ (function (_super) {
        ...
        this.registerEvent('mouseMoveEvent', function (event) {
            ...
            if (progressInstanceInfo !== null) {
                ...
                if (overlay.isDrawing() && progressInstancePaneId === paneId) {
                    var dataList = pane.getChart().getChartStore().getDataList();
                    overlay.eventMoveForDrawing(_this._coordinateToPoint(progressInstanceInfo.instance, event), dataList);
            ...
            if (progressInstanceInfo !== null) {
                ...
                if (overlay.isDrawing() && progressInstancePaneId === paneId) {
                    var dataList = pane.getChart().getChartStore().getDataList();
                    overlay.eventMoveForDrawing(_this._coordinateToPoint(overlay, event), dataList);
                    (_a = overlay.onDrawing) === null || _a === void 0 ? void 0 : _a.call(overlay, __assign({ overlay: overlay, figureKey: key, figureIndex: index }, event));
                    overlay.nextStep();
    ...
    OverlayView.prototype._figureMouseRightClickEvent = function (overlay, _figureType, figureKey, figureIndex, _attrsIndex) {
        return function (event) {
            var _a;
            (_a = overlay.onRightClick) === null || _a === void 0 ? void 0 : _a.call(overlay, __assign({ overlay: overlay, figureIndex: figureIndex, figureKey: figureKey }, event));
            return true;
        };
    };
...
var SessionBreakView = /** @class */ (function (_super) {
    __extends(SessionBreakView, _super);
    function SessionBreakView() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    SessionBreakView.prototype.drawImp = function (ctx) {
        var _this = this;
        var widget = this.getWidget();
        var pane = widget.getPane();
        var chartStore = pane.getChart().getChartStore();
        var bounding = widget.getBounding();
        var lineStyle = chartStore.getStyles().sessionBreak;
        this.eachChildren(function (data) {
            var _a;
            var kLineData = data.data; data.x;
            if (isValid(kLineData)) {
                var canDrawAt = kLineData.isStartOfSession;
                if (canDrawAt) {
                    (_a = _this.createFigure({
                        name: "line",
                        attrs: {
...
var CandleWidget = /** @class */ (function (_super) {
    ...
    function CandleWidget(rootContainer, pane) {
        ...
        _this._sessionBreakView = new SessionBreakView(_this);
    ...
    CandleWidget.prototype.updateMainContent = function (ctx) {
        ...
        var chartStore = this.getPane().getChart().getChartStore();
        var settings = chartStore.getTradingSettings();
        var timeframe = chartStore.getTimeframeDuration();
...
var AxisImp = /** @class */ (function () {
    function AxisImp(parent) {
        this._defaultRange = { from: 0, to: 0, range: 0, realFrom: 0, realTo: 0, realRange: 0 }; //to help reset to initial value
    ...
    AxisImp.prototype.reset = function () {
...
var YAxisImp = /** @class */ (function (_super) {
    ...
    YAxisImp.prototype.resetRange = function () {
        this.reset();
    };
...
var Event = /** @class */ (function () {
    ...
    Event.prototype.mouseRightClickEvent = function (e) {
        ...
        if (widget !== null) {
            ...
            this._chart.getChartStore().getActionStore().execute(exports.ActionType.onRightClick, {
                name: name_5,
                x: e.pageX,
                y: e.pageY,
            });
...
var ChartImp = /** @class */ (function () {
    ...
    ChartImp.prototype.applyNewData = function (data, more, callback) {
        ...
        var mutableArray = __spreadArray([], __read(data), false);
        this._chartStore.addData(mutableArray, LoadDataType.Init, more).then(function () { }).catch(function () { }).finally(function () { callback === null || callback === void 0 ? void 0 : callback(); });
    };
    /**
     * @deprecated
     * Since v9.8.0 deprecated, since v10 removed
    ...
    ChartImp.prototype.applyMoreData = function (data, more, callback) {
        ...
        var mutableArray = __spreadArray([], __read(data), false);
        this._chartStore.addData(mutableArray, LoadDataType.Forward, more !== null && more !== void 0 ? more : true).then(function () { }).catch(function () { }).finally(function () { callback === null || callback === void 0 ? void 0 : callback(); });
    };
    ...
    ChartImp.prototype.createIndicator = function (value, isStack, paneOptions, callback) {
        ...
        if (getIndicatorClass(indicator.name) === null) {
            logWarn('createIndicator', 'value', "indicator not supported ".concat(value, ", you may need to use registerIndicator to add one!!!"));
            return null;
        }
    ...
    ChartImp.prototype.resetScale = function () {
        var _this = this;
        this._drawPanes.forEach(function (pane) {
    ...
    ChartImp.prototype.setTradingSettings = function (settings) {
        var _a;
        this._chartStore.setTradingSettings(settings);
        this.updatePane(0 /* UpdateLevel.Main */, (_a = this._candlePane) === null || _a === void 0 ? void 0 : _a.getId());
    };
    ChartImp.prototype.setTimeframe = function (timeFrame) {
    ...
    ChartImp.prototype.updateDataIntraDay = function (data, callback) {
        this._chartStore.addIntradayData(data).then(function () { }).catch(function () { }).finally(function () { callback === null || callback === void 0 ? void 0 : callback(); });
    };
    return ChartImp;
}());
